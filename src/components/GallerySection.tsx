      ...
      {selectedPhoto && (
        <div className="fixed inset-0 z-50 bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-4">
          <motion.div
            ...
            className="bg-white rounded-3xl max-w-3xl w-full overflow-hidden shadow-2xl border border-slate-700 relative"
          >
            <button
            ...
            <div className="p-6 space-y-3 bg-white">
              <div className="flex items-center gap-2 text-xs">
                ...
                <span className="text-slate-500 font-medium">{selectedPhoto.date}</span>
              </div>
        ...
        {selectedVideo && (
          <div className="fixed inset-0 z-50 bg-slate-950/95 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
              className="bg-slate-900 rounded-3xl max-w-4xl w-full overflow-hidden shadow-2xl border border-slate-700 relative text-white my-8"
            >
              {/* Close Button */}
              <button
                onClick={() => setSelectedVideo(null)}
                className="absolute top-4 right-4 z-20 p-2.5 rounded-full bg-black/60 hover:bg-black text-white transition-colors cursor-pointer shadow-lg"
              >
                <X className="w-5 h-5" />
              </button>
              ...
              <div className="relative w-full aspect-video bg-black flex items-center justify-center overflow-hidden">
                {selectedVideo.platform === 'direct' ? (
                  ...
                  <iframe
                    src={getVideoEmbedUrl(selectedVideo.videoUrl, selectedVideo.platform) || selectedVideo.videoUrl}
                  ...
                  <div className="flex flex-wrap items-center gap-2 text-xs">
                    <span
                      className={`px-3 py-1 rounded-xl text-[11px] font-black uppercase tracking-wider ${
                        getVideoPlatformInfo(selectedVideo.platform).badgeColor
                      }`}
                    >
                      {getVideoPlatformInfo(selectedVideo.platform).label}
                    </span>
                    <span className="px-3 py-1 rounded-xl bg-white/10 text-amber-300 font-bold">
